...
async def parse_manager_command(
    ...
    if upper.startswith("SCHED_APPROVE_"):
        ...
        return await cmd_scheduled_token_approve(restaurant_id, manager_phone, token_id)
    if upper.startswith("SCHED_REJECT_"):
        token_id = raw[len("SCHED_REJECT_"):]
        return await cmd_scheduled_token_reject(restaurant_id, manager_phone, token_id)
...
async def cmd_scheduled_token_approve(
    restaurant_id: str, manager_phone: str, token_id: str,
) -> Dict[str, Any]:
    """Approve scheduled delivery or takeaway from manager WhatsApp button."""
    from tools.db_tools import get_walk_in_token_by_id
    from tools.booking_mechanisms import (
        approve_scheduled_delivery_token,
        approve_scheduled_takeaway_token,
    )

    token = await get_walk_in_token_by_id(restaurant_id, token_id)
    token_type = (token or {}).get("type")
    if token_type == "scheduled_takeaway":
        result = await approve_scheduled_takeaway_token(restaurant_id, token_id)
        label = "takeaway"
    else:
    ...
    if result.get("ok"):
        await send_whatsapp_message(
            manager_phone,
            f"✅ Scheduled {label} *{token_id}* approved. Customer will receive payment link.",
            restaurant_id,
        )
...
async def cmd_scheduled_token_reject(
    restaurant_id: str, manager_phone: str, token_id: str,
) -> Dict[str, Any]:
    """Reject scheduled delivery or takeaway from manager WhatsApp button."""
    from tools.db_tools import get_walk_in_token_by_id
    from tools.booking_mechanisms import (
        reject_scheduled_delivery_token,
        reject_scheduled_takeaway_token,
    )

    token = await get_walk_in_token_by_id(restaurant_id, token_id)
    token_type = (token or {}).get("type")
    if token_type == "scheduled_takeaway":
        result = await reject_scheduled_takeaway_token(restaurant_id, token_id)
        label = "takeaway"
    else:
    ...
    if result.get("ok"):
        await send_whatsapp_message(
            manager_phone,
            f"❌ Scheduled {label} *{token_id}* rejected. Customer notified.",
            restaurant_id,
        )
        return {"status": "success", "token_id": token_id}
    await send_whatsapp_message(
