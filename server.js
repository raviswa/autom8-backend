...
// GET /api/dashboard/waba — WABA info from Munafe Chat restaurants table
app.get('/api/dashboard/waba', authenticateToken, getRestaurantId, async (req, res) => {
  try {
    ...
    // Try to find matching restaurant by id, then fall back to first record
    let data = null;
...
// GET /api/dashboard/wa-orders — WhatsApp orders from Munafe Chat bookings
app.get('/api/dashboard/wa-orders', authenticateToken, getRestaurantId, async (req, res) => {
  try {
    ...
    // Try bookings with customer join
    const { data, error } = await supabaseChat
      .from('bookings')
      .select('id, created_at, service_type, status, total_amount, party_size, token_number, customer_id, customers(name, phone)')
    ...
    const { data: plain, error: e2 } = await supabaseChat
      .from('bookings')
    ...
    if (!e2 && plain) {
      return res.json({ success: true, orders: plain });
    }
    ...
    res.json({ success: true, orders: evts ?? [] });
  } catch (err) {
