...
//   psl             — flat PRODUCT rows PLUS optional size-variant linking
//                      (item_type / variant_group_id / size_label)
...
// TOPPINGS / CRUST
// ────────────────
// Deliberately NOT part of this schema. Per-outlet shared, size-tiered
// add-on lists (toppings, crust upgrades) are the job of the existing
// menu-customization modal / option-group system, not the catalog upload
// path. Menu upload only creates the base priced items; customization is
...
export const LOB_SCHEMAS = {
  ...
  // ══════════════════════════════════════════════════════════════════════
  // food_products — flat PRODUCT rows. No new DB columns; extra
  // pack-size/shelf-life/ingredients fields are NOT persisted in this pass
  // (no live tenant on this LOB yet — add if/when one exists).
  // ══════════════════════════════════════════════════════════════════════
  food_products: {
    id: 'food_products',
    ...
    columnHelp: [
      ...
      ['image_link — direct image URL; leave blank if you have no photo yet'],
  ...
  // ══════════════════════════════════════════════════════════════════════
  // retail — flat PRODUCT rows. Same reasoning as food_products.
  // ══════════════════════════════════════════════════════════════════════
  retail: {
    id: 'retail',
  ...
  // ══════════════════════════════════════════════════════════════════════
  // psl — Pizza & Ice Cream (mixed outlet). Flat PRODUCT rows by default;
  // OPTIONAL size-variant linking via item_type/variant_group_id/size_label
  // for pizzas/shakes that come in multiple sizes at one price per size
  // (see: Pizza Hut / Darbar-e / Aroma menu references — one product name,
  // 2-4 size-price columns).
  //
  // Toppings, crust upgrades, ice-cream flavours are NOT part of this
  // schema — those are shared, size-tiered, per-outlet option lists
  // configured through the existing customization modal, not the upload.
  // ══════════════════════════════════════════════════════════════════════
  psl: {
    id: 'psl',
    ...
    templateColWidths: [
      { wch: 10 }, { wch: 10 }, { wch: 14 }, { wch: 10 }, { wch: 26 }, { wch: 40 }, { wch: 8 }, { wch: 16 }, { wch: 48 }, { wch: 12 },
    ],
    templateExamples: [
      // Linked size variants — same variant_group_id, three rows.
      ...
      // Flat product — no size, no variant_group_id.
    ...
    columnHelp: [
      ...
      [''],
      ['item_type — PIZZA for size-linked pizzas, PRODUCT for everything else (ice cream, sides, beverages, desserts)'],
      ['variant_group_id — same value across all sizes of ONE pizza (e.g. PZ001 for Margherita Small/Medium/Large). Leave blank for flat PRODUCT rows.'],
      ['size_label — Small / Medium / Large etc. Leave blank for flat PRODUCT rows.'],
      ['Toppings, crust upgrades, and flavour pickers are configured separately in menu customization — not in this upload.'],
    ],
    previewColumns: [
      { key: 'id',               label: 'SKU',   mono: true,  width: '8%'  },
    ...
    validateRow(item, rowNum) {
      ...
      if (item.item_type === 'PIZZA' && !item.variant_group_id) {
        errors.push(`Row ${rowNum} (${item.name || item.id}): PIZZA rows need a variant_group_id so sizes link together`);
      }
      if (item.variant_group_id && !item.size_label) {
        errors.push(`Row ${rowNum} (${item.name || item.id}): has a variant_group_id but no size_label`);
      }
      return errors;
    },
  },
